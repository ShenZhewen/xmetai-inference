...
def run_inference(model,  input):
    ...
    print("[DEBUG] Inference batch shape:", batch.shape, "dtype:", batch.dtype)
    ...
    for member in range(total_member):
        ...
        for t in range(total_step):
            ...
            print(f"[DEBUG] member={member:03d} step={t+1:03d} new_input.shape={new_input.shape} output.shape={output.shape} output.dtype={output.dtype}")
...
if __name__ == "__main__":
    ...
    print("[DEBUG] ONNX Runtime version:", ort.__version__)
    print("[DEBUG] Available providers:", ort.get_available_providers())
    ...
    print("[DEBUG] Active session providers:", model.get_providers())
    print("[DEBUG] Model input names:", input_names)
    print("[DEBUG] Model input shapes:", [(x.name, x.shape) for x in model.get_inputs()])
    print("[DEBUG] Model output names:", [x.name for x in model.get_outputs()])
    print("[DEBUG] Model output shapes:", [(x.name, x.shape) for x in model.get_outputs()])
    print("[DEBUG] Input DataArray shape:", input.shape, "dtype:", input.dtype)
    print("[DEBUG] Input time values:", input.time.values)
    print("[DEBUG] Input channel count:", len(input.channel))
    print("[DEBUG] Input lat:", input.lat.values[0], input.lat.values[-1])
    print("[DEBUG] Input lon:", input.lon.values[0], input.lon.values[-1])
