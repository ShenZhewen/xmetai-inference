...
推理的 run 循环**默认每隔几步打一条进度**（不用 `--verbose`），行内只放关键信息：
已预报几天 / 剩余几天 / 预计还要多久。四卡并发时每条是**完整一行**（带换行），不会互相覆盖：

```
[rank 0/4] 0106 预报 5.0/15.2 天 · 剩 10.2 天 · 还要 29m44s
[rank 0/4] 起报 1/89 (  1%) 完成，耗时 46m09s（剩 88 个起报，总预计还要 ~67h47m）
```

`--verbose` 才回到逐步耗时明细（每步精确耗时）。
